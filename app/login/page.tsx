  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      if (error.code === 'auth/account-exists-with-different-credential') {
        setAuthError("Există deja un cont cu acest email. Încearcă cu altă metodă de login.");
      } else {
        setAuthError("Eroare la autentificare. Încearcă din nou.");
      }
    }
  };
    ...
    <div className="min-h-screen bg-[#09090b] flex flex-col items-center p-4 font-sans relative overflow-y-auto overflow-x-hidden">
      ...
      <div className="w-full max-w-md p-8 md:p-12 bg-zinc-900/80 backdrop-blur-md rounded-3xl border border-zinc-800 shadow-2xl relative z-10 flex flex-col items-center my-12 shrink-0">
        ...
        <p className="text-zinc-400 text-center mb-8 font-medium text-sm">
          {isLoginMode && !isForgotMode && "Bine ai revenit! Intră în contul tău."}
          {!isLoginMode && "Creează-ți contul gratuit în câteva secunde."}
          {isForgotMode && "Îți trimitem un link de resetare a parolei."}
        </p>
        ...
        {!isForgotMode && (
          <div className="w-full flex items-center gap-4 mb-6">
            ...
            <span className="text-zinc-600 text-xs font-medium tracking-widest uppercase">sau</span>
            <div className="flex-1 h-px bg-zinc-800"></div>
          </div>
        )}
