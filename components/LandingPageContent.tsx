    ...
    >
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          ...
          >
            ...
            <span className="relative flex items-center gap-2">
              {t('generatePlanNow', locale)} <span className="group-hover:translate-x-1 transition-transform">&rarr;</span>
            </span>
          </Link>
          <p className="text-sm text-zinc-500 mt-4 sm:mt-0 sm:ml-4 font-medium">
            {t('noCardRequired', locale)}
          </p>
        </div>

        {/* Free tier communicate */}
        <div className="flex flex-wrap justify-center gap-x-8 gap-y-2 mt-8 text-sm text-zinc-500 font-medium">
          <span className="flex items-center gap-1.5">
            <span className="text-emerald-400 font-bold">✓</span> {t('noCardLabel', locale)}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="text-emerald-400 font-bold">✓</span> {t('freePlansDemo', locale)}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="text-emerald-400 font-bold">✓</span> {t('freePlansStudio', locale)}
          </span>
        </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-xl p-2 md:p-4 shadow-2xl relative overflow-hidden transform perspective-1000 rotate-x-12 scale-95 opacity-80 hover:opacity-100 hover:scale-100 transition-all duration-700 ease-out">
            ...
            <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
              <div className="bg-zinc-900/90 border border-emerald-500/30 p-6 rounded-2xl backdrop-blur-md shadow-2xl">
                <h3 className="text-white font-bold text-xl mb-2">{t('successGenerated', locale)}</h3>
                <div className="flex gap-2 items-center text-emerald-400">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="font-medium text-sm">{t('readyToDownload', locale)}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative z-10 py-24 px-6 border-t border-zinc-800/50 bg-zinc-900/20">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-black text-center mb-16">{t('everythingInOnePlace', locale)}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-zinc-800/20 border border-zinc-800 p-8 rounded-3xl hover:bg-zinc-800/40 transition-colors group">
              ...
              <h3 className="text-xl font-bold text-white mb-3">{t('amazingSpeed', locale)}</h3>
              <p className="text-zinc-400 leading-relaxed">{t('amazingSpeedDesc', locale)}</p>
            </div>
            ...
            <div className="bg-zinc-800/20 border border-zinc-800 p-8 rounded-3xl hover:bg-zinc-800/40 transition-colors group">
              ...
              <h3 className="text-xl font-bold text-white mb-3">{t('perfectFinancials', locale)}</h3>
              <p className="text-zinc-400 leading-relaxed">{t('perfectFinancialsDesc', locale)}</p>
            </div>
            ...
            <div className="bg-zinc-800/20 border border-zinc-800 p-8 rounded-3xl hover:bg-zinc-800/40 transition-colors group">
              ...
              <h3 className="text-xl font-bold text-white mb-3">{t('absoluteSecurity', locale)}</h3>
              <p className="text-zinc-400 leading-relaxed">{t('absoluteSecurityDesc', locale)}</p>
            </div>
          </div>
        </div>
      </section>
