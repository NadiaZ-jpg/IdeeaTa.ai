...
export default function LoginContent({ locale = "ro" }: { locale?: "ro" | "en" }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        router.push(isEn ? '/en/dashboard' : '/dashboard');
      } else {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        ...
        setAuthError(error.message || (isEn ? "An error occurred. Please try again." : "A apărut o eroare. Încearcă din nou."));
      }
    } finally {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
        setAuthError(isEn ? "Incorrect email or password." : "Email sau parolă incorectă.");
      } else if (error.code === 'auth/email-already-in-use') {
        setAuthError(isEn ? "An account already exists with this email. Please log in." : "Există deja un cont cu acest email. Te rugăm să te loghezi.");
      } else if (error.code === 'auth/weak-password') {
        setAuthError(isEn ? "Password must be at least 6 characters long." : "Parola trebuie să aibă cel puțin 6 caractere.");
      } else {
        setAuthError(error.message || (isEn ? "An unknown authentication error occurred." : "A apărut o eroare necunoscută la autentificare."));
      }
      setIsEmailLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#09090b] flex flex-col items-center p-4 font-sans relative overflow-y-auto overflow-x-hidden">
      ...
      <div className="w-full max-w-md p-8 md:p-12 bg-zinc-900/80 backdrop-blur-md rounded-3xl border border-zinc-800 shadow-2xl relative z-10 flex flex-col items-center my-12 shrink-0">
        ...
        <p className="text-zinc-400 text-center mb-8 font-medium text-sm">
          {isLoginMode && !isForgotMode && (isEn ? "Welcome back! Log in to your account." : "Bine ai revenit! Intră în contul tău.")}
          {!isLoginMode && (isEn ? "Create your free account in a few seconds." : "Creează-ți contul gratuit în câteva secunde.")}
          {isForgotMode && (isEn ? "We will send you a password reset link." : "Îți trimitem un link de resetare a parolei.")}
        </p>
        ...
        {!isForgotMode && (
          <div className="w-full flex flex-col gap-3 mb-6">
            ...
            >
              ...
              {isEn ? "Continue with Google" : "Continuă cu Google"}
            </button>
            ...
            >
              ...
              {isEn ? "Continue with Facebook" : "Continuă cu Facebook"}
            </button>
          </div>
        )}
        ...
        {!isForgotMode && (
          <div className="w-full flex items-center gap-4 mb-6">
            ...
            <span className="text-zinc-600 text-xs font-medium tracking-widest uppercase">{isEn ? "or" : "sau"}</span>
            <div className="flex-1 h-px bg-zinc-800"></div>
          </div>
        )}
        ...
        <form onSubmit={handleEmailAuth} className="w-full mb-4 space-y-4">
          <div>
            <input 
              ...
              name="email"
              type="email" 
              placeholder={isEn ? "Email address" : "Adresa de email"}
              value={email}
          ...
          {!isForgotMode && (
            <div>
              <input 
                ...
                name="password"
                type="password" 
                placeholder={isEn ? "Password" : "Parola"}
                value={password}
          ...
          >
            {isEmailLoading 
              ? (isEn ? "Processing..." : "Se procesează...") 
              : isForgotMode 
                ? (isEn ? "Send reset link" : "Trimite link de resetare") 
                : (isLoginMode ? (isEn ? "Log In" : "Intră în cont") : (isEn ? "Create Account" : "Creează cont nou"))}
          </button>
          ...
          {resetEmailSent && (
            <div className="w-full p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              <p className="text-emerald-400 text-sm font-medium text-center">
                {isEn ? "✅ Reset link has been sent! Check your email." : "✅ Link-ul de resetare a fost trimis! Verifică-ți emailul."}
              </p>
            </div>
          )}
          ...
          <div className="flex justify-between items-center mt-2 text-sm">
            {isLoginMode && !isForgotMode && (
              ...
              >
                {isEn ? "Forgot password?" : "Ai uitat parola?"}
              </button>
            )}
            {isForgotMode && (
              <button 
                ...
                className="text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer"
              >
                {isEn ? "← Back to login" : "← Înapoi la login"}
              </button>
            )}
            {!isForgotMode && (
              <button 
              ...
              >
                {isLoginMode 
                  ? (isEn ? "Don't have an account? Sign up" : "Nu ai cont? Creează unul nou") 
                  : (isEn ? "Already have an account? Log in" : "Ai deja cont? Intră aici")}
              </button>
            )}
          </div>
        </form>
        ...
        <div className="w-full mt-8 pt-8 border-t border-zinc-800 flex items-center gap-5">
          ...
          <div>
            <p className="text-white font-bold text-sm mb-1.5">
              {isEn ? "Test on phone 📱" : "Testează pe telefon 📱"}
            </p>
      ...
      <div className="w-full max-w-4xl mt-4 mb-20 px-8 py-16 bg-zinc-900/40 border border-zinc-800/80 rounded-3xl relative z-10 flex flex-col gap-10 text-zinc-300 backdrop-blur-sm">
        <div className="text-center">
          <h2 className="text-3xl font-black text-white mb-4">
            {isEn ? "Intelligent Business Plan Generation" : "Generare Inteligentă de Planuri de Afaceri"}
          </h2>
