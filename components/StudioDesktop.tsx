...
export default function StudioDesktop({ locale = "ro" }: { locale?: "ro" | "en" | "es" }) {
  ...
  const handleAiEdit = async (
    ...
    // One-time Pro pack remaining (client pre-check; server is source of truth)
    ...
    const { isCombine, baseSource, currentStack } = resolveEditBaseForToolRun({
      ...
      forceSiblingFromOriginal: isToneAction,
    });
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (user) {
        // Cont logat: nu bloca Altă idee / Otra idea pe popstate
  ...
  const generate = async (e?: React.FormEvent, retryCount = 0) => {
    ...
    if (retryCount === 0) {
      ...
      // LIMITATOR STUDIO — free + Pro pack generations
      if (user && !isAdmin) {
        const canGen = canGenerateWithQuotas({
          isPaid,
          subscriptionActive,
          lifetimePlanCount,
    ...
    >
      ...
      <PricingModal
        ...
        onSuccess={() => {
          ...
          // Resetare contori limitatoare după plată
          if (typeof window !== 'undefined') {
      ...
      {showExpertDrawer && (
        <ExpertSectionsDrawer
          ...
          onAddSection={(newSection) => {
            ...
            syncCurrentPlanToFirestore(applied.plan, applied.versions);
            if (typeof window !== "undefined") {
