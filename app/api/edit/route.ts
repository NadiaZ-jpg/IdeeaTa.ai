...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "professional_tone") {
      ...
      instruction = "Adaugă 2 concepte suplimentare la 'functionalitati_ai' și extinde secțiunea 'amenintari' din analiza_swot cu încă o amenințare relevantă. Păstrează tonul existent. Nu adăuga chei noi în JSON care nu sunt în modelul inițial.";
...
Returnează DOAR rezultatul ca un JSON valid, respectând fix aceeași schemă (nume, slogan, descriere, analiza_swot, buget_detaliat, functionalitati_ai). Include toate lucrurile existente, doar că modificate sau îmbogățite conform instrucțiunii.
