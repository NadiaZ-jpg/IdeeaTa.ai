"use client";
/**
 * MockupPreview — exemplu static "Urban Beans" (înainte de primul plan).
 * Tab-uri interne pe chei stabile (summary/swot/budget/strategy), etichete din ui (RO/EN/ES).
 */

import React from "react";

export type MockupInnerTab = "summary" | "swot" | "budget" | "strategy";
...
export function MockupPreview({
  ...
  const innerTabs: { key: MockupInnerTab; label: string }[] = [
    { key: "summary", label: ui.mockupSummary },
    { key: "swot", label: ui.mockupSwot },
    { key: "budget", label: ui.mockupBudget },
    { key: "strategy", label: ui.mockupStrategy },
  ];

  const activeInner = (innerMockupTab || "swot") as MockupInnerTab;
    ...
    <>
        ...
        <div className="relative border border-zinc-800/60 rounded-[2.5rem] bg-[#09090b] overflow-hidden shadow-2xl ring-1 ring-white/5 min-h-[500px]">
          <div className="flex gap-1 border-b border-zinc-800/80 px-6 pt-5 pb-0 bg-zinc-900/40">
            ...
            {innerTabs.map((tab) => (
              <div
                key={tab.key}
                onClick={() => setInnerMockupTab(tab.key)}
          ...
          <div className="p-8 md:p-12 animate-in fade-in duration-300">
            <div className="mb-8">
              <div className="text-emerald-500 font-bold text-sm tracking-wider uppercase mb-2">
                {ui.generatedExample}
              </div>
            ...
            {activeInner === "summary" && (
              <div className="text-zinc-400 leading-relaxed text-lg animate-in slide-in-from-bottom-2">
                <p className="mb-4">
                  <strong>Urban Beans</strong> {ui.mockupSummaryP1}
                </p>
                <p>{ui.mockupSummaryP2}</p>
              </div>
            )}
            ...
            {activeInner === "budget" && (
              <div className="animate-in slide-in-from-bottom-2 bg-zinc-900/50 rounded-2xl p-6 border border-zinc-800">
                <h5 className="font-bold text-emerald-400 mb-6 text-lg">{ui.mockupBudgetTitle}</h5>
                <div className="space-y-4">
                  <div className="flex justify-between items-center pb-3 border-b border-zinc-800">
                    <span className="text-zinc-400">{ui.mockupBudgetEq}</span>
                    <span className="font-mono text-zinc-200">{ui.mockupBudgetEqVal}</span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-zinc-800">
                    <span className="text-zinc-400">{ui.mockupBudgetDes}</span>
                    <span className="font-mono text-zinc-200">{ui.mockupBudgetDesVal}</span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-zinc-800">
                    <span className="text-zinc-400">{ui.mockupBudgetStock}</span>
                    <span className="font-mono text-zinc-200">{ui.mockupBudgetStockVal}</span>
                  </div>
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-zinc-300 font-bold">{ui.mockupBudgetTotal}</span>
                    <span className="font-mono text-emerald-400 font-bold text-xl">{ui.mockupBudgetTotalVal}</span>
                  </div>
                </div>
              </div>
            )}

            {activeInner === "strategy" && (
              <div className="animate-in slide-in-from-bottom-2 text-zinc-400 space-y-6">
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{ui.mockupStrategy1_Desc}</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{ui.mockupStrategy2_Desc}</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h6 className="text-white font-bold mb-1">{ui.mockupStrategy3_Title}</h6>
                    <p className="text-sm">{ui.mockupStrategy3_Desc}</p>
                  </div>
                </div>
              </div>
            )}
      ...
      {mockupTab === 1 && (
        <div className="relative border border-zinc-800/60 rounded-[2.5rem] bg-[#09090b] overflow-hidden shadow-2xl ring-1 ring-white/5 p-8 md:p-12 min-h-[420px] animate-in fade-in duration-300">
          <div>
            <h4 className="text-xl font-bold text-white mb-2">{ui.mockupFinYearTitle}</h4>
            <p className="text-zinc-400 mb-8 text-sm">{ui.mockupFinYearDesc}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              <div className="bg-zinc-900/50 rounded-2xl border border-zinc-800 p-6 flex flex-col justify-between">
                <h5 className="text-emerald-400 font-bold mb-6 text-sm uppercase tracking-wider">{ui.mockupChartRevenue}</h5>
                <div className="grid grid-cols-4 gap-4 h-48 items-end">
                  {[
                    ...
                    <div key={i} className="flex flex-col items-center gap-3 h-full justify-end group">
                      <span
                        className="text-xs text-zinc-500 group-hover:text-white transition-colors opacity-0 group-hover:opacity-100 absolute -translate-y-8 animate-float-number"
                        style={{ animationDelay: `${i * 150 + 500}ms` }}
                      >
                        {bar.val * 5}k
                      </span>
                      <div
                        className="w-full bg-zinc-800 rounded-t-xl overflow-hidden flex items-end shadow-inner relative group-hover:bg-zinc-700 transition-colors"
                        style={{ height: "80%" }}
                      >
                        <div
                          className={`w-full rounded-t-xl animate-scale-y ${
                            i === 3 ? "bg-emerald-400" : i === 2 ? "bg-emerald-500" : "bg-emerald-600/80"
                          }`}
              ...
              <div className="flex flex-col gap-6">
                <div className="flex-1 bg-zinc-900/50 rounded-2xl border border-zinc-800 p-6">
                  <h5 className="text-orange-400 font-bold mb-4 text-sm uppercase tracking-wider">{ui.mockupChartCostsDist}</h5>
                  {[
                    { label: ui.mockupChartSalaries, w: 85, color: "bg-orange-500" },
                    { label: ui.mockupChartRent, w: 60, color: "bg-orange-400/80" },
                    { label: ui.mockupChartStock, w: 45, color: "bg-orange-300/60" },
                    ...
                    <div key={i} className="flex items-center gap-4 mb-3 last:mb-0 group cursor-pointer">
                      <div className="w-28 text-xs font-semibold text-zinc-400 group-hover:text-zinc-200 transition-colors truncate">
                        {item.label}
                      </div>
                      <div className="flex-1 bg-zinc-800 rounded-full h-3 overflow-hidden shadow-inner">
                        <div
                          className={`${item.color} h-full rounded-full animate-scale-x group-hover:brightness-125`}
                          style={{ width: `${item.w}%`, animationDelay: `${i * 150 + 400}ms` }}
                        ></div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="bg-zinc-900/50 rounded-2xl border border-zinc-800 p-6 flex items-center justify-between">
                  <div>
                    <h5 className="text-blue-400 font-bold mb-1 text-sm uppercase tracking-wider">{ui.mockupProfitMargin}</h5>
                    <p className="text-3xl font-black text-white">
                      24<span className="text-lg text-zinc-500">%</span>
                    </p>
                  </div>
                  <div className="w-24 h-24 rounded-full border-[8px] border-zinc-800 relative flex items-center justify-center shrink-0 shadow-[0_0_20px_rgba(59,130,246,0.1)]">
                    <div
                      ...
                      style={{ background: "conic-gradient(#3b82f6 0% 24%, transparent 24% 100%)" }}
                    ></div>
                ...
                {[
                  ...
                  <div key={i} className="flex items-center gap-4 text-xs font-mono">
                    ...
                    <div className="flex-1 bg-zinc-800/50 h-1.5 rounded-full overflow-hidden">
                      <div className={`${item.color} h-full`} style={{ width: `${item.w}%` }}></div>
                    </div>
        ...
        <div className="relative border border-zinc-800/60 rounded-[2.5rem] bg-[#09090b] overflow-hidden shadow-2xl ring-1 ring-white/5 min-h-[420px]">
          <div className="grid grid-cols-2 h-full min-h-[360px]">
            <div className="p-8 border-r border-zinc-800/60 opacity-60">
              <div className="text-zinc-500 text-xs font-bold uppercase tracking-widest mb-4">{ui.mockupBeforeTitle}</div>
            ...
            <div className="p-8 opacity-50">
              ...
              <div className="flex flex-col gap-2">
                {[ui.swotFull, ui.budget12m, ui.marketStrategy, ui.euFundsEligibility, ui.exportPdfPptx].map(
                  (item: string) => (
                    <div
                      key={item}
                      className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2 text-xs text-emerald-300 font-medium"
                    >
                      {item}
                    </div>
                  )
